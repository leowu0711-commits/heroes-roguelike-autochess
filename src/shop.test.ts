...
describe("shop and player economy", () => {
  ...
  it("buying and selling updates gold with half sell value, bench, and pool stock", () => {
    const pool = createInitialPool([{ id: "jing-ke", cost: 2, copies: 45 }]);
    const player = createPlayerState({ gold: 5 });
    const bought = buyShopHero({ player, pool, heroId: "jing-ke", cost: 2 });

    assert.equal(bought.gold, 3);
  ...
  it("grants natural xp and buys 4 xp for 5 gold using the original level curve", () => {
